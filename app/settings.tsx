...
export default function SettingsScreen() {
  ...
  return <ScreenShell title="Settings" eyebrow={workspace?.name || "Lumière House"} subtitle={user?.email || "Account settings"}>
    <View style={{ gap: 16 }}>
      ...
      <View style={{ gap: 10 }}><SecondaryButton label="Open Results" onPress={() => router.push("/results" as any)} icon="insights" /><SecondaryButton label="Import performance evidence" onPress={() => router.push("/performance" as any)} icon="upload-file" /><SecondaryButton label="Manage schedules" onPress={() => router.push("/schedules" as any)} icon="schedule" /><SecondaryButton label="Read help and policy notes" onPress={() => router.push("/help" as any)} icon="help-outline" /><SecondaryButton label="Sign out" onPress={signOut} icon="logout" /></View>
    </View>
  </ScreenShell>;
}
