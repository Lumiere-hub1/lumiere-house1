  ...
  return <ScreenShell title="Set up your house" eyebrow={`Step ${step} of 4`} subtitle="Start with the essentials. You can refine your business memory later." scroll={false}>
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
      <View style={{ gap: 16, paddingTop: 22 }}>
        ...
        {step === 2 ? <><Text style={{ color: brand.ink, fontSize: 18, fontWeight: "700" }}>What do you sell?</Text><Field label="Products or services" value={products} onChangeText={setProducts} placeholder="Describe the offer in your own words" multiline numberOfLines={5} textAlignVertical="top" /></> : null}
        {step === 3 ? <><Text style={{ color: brand.ink, fontSize: 18, fontWeight: "700" }}>Who do you serve?</Text><Field label="Target customer" value={audience} onChangeText={setAudience} placeholder="The people you most want to reach" multiline numberOfLines={5} textAlignVertical="top" /></> : null}
        {step === 4 ? <><Text style={{ color: brand.ink, fontSize: 18, fontWeight: "700" }}>What do you want next?</Text><View style={{ gap: 8 }}>{outcomes.map((item) => <Pressable key={item.value} accessibilityRole="radio" accessibilityState={{ selected: outcome === item.value }} onPress={() => setOutcome(item.value)} style={({ pressed }) => [{ minHeight: 46, borderRadius: 13, borderWidth: 1, borderColor: outcome === item.value ? brand.rose : brand.border, paddingHorizontal: 14, justifyContent: "center", backgroundColor: outcome === item.value ? `${brand.rose}18` : "transparent" }, pressed && { opacity: 0.75 }]}><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}><Text style={{ color: brand.ink, fontSize: 14, fontWeight: outcome === item.value ? "700" : "500" }}>{item.label}</Text>{outcome === item.value ? <StatusPill label="Selected" tone="success" /> : null}</View></Pressable>)}</View><Field label="Target" hint="Use a count for now; you can add revenue assumptions in Growth." keyboardType="number-pad" value={target} onChangeText={setTarget} placeholder="30" /></> : null}
