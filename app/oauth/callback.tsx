...
export default function OAuthCallback() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      ...
      try {
        // Session tokens are only accepted after the backend resolves the user.
        if (params.sessionToken) {
          console.log("[OAuth] Session token found in params (web callback)");
          await Auth.setSessionToken(params.sessionToken);
          const resolvedUser = await Api.getMe();
          if (!resolvedUser) throw new Error("The returned session could not be verified.");
          setStatus("success");
        ...
        if (sessionToken) {
          ...
          const resolvedUser = await Api.getMe();
          if (!resolvedUser) throw new Error("The returned session could not be verified.");
          console.log("[OAuth] Session resolved by backend");
          setStatus("success");
