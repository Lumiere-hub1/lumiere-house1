...
export default function ClientCareScreen() {
  ...
  return <ScreenShell title="Client Care" eyebrow={workspace.name} subtitle="Remember the relationship, respect the permission, and follow up with intention." action={<StatusPill label={`${consentedCount} consented`} tone={consentedCount ? "success" : "neutral"} />}>
    <View style={{ gap: 16 }}>
      <Surface tone="ink" style={{ borderColor: brand.ink }}><Text style={{ color: brand.cream, fontSize: 17, fontWeight: "700" }}>Helpful, never intrusive.</Text><Text style={{ color: "#D8C9BA", fontSize: 13, lineHeight: 19 }}>Outbound recommendations respect consent, quiet hours, customer preferences, and workspace approval rules.</Text></Surface>
