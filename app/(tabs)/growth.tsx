...
export default function GrowthScreen() {
  ...
  return <ScreenShell title="Growth" eyebrow={workspace.name} subtitle="Make the business outcome explicit. Lumière will keep assumptions separate from facts." action={<StatusPill label={`${goals.data?.length || 0} goal${goals.data?.length === 1 ? "" : "s"}`} tone="neutral" />}>
    <View style={{ gap: 16 }}>
      ...
      {showCampaignForm ? <Surface><View style={{ gap: 14 }}><Field label="Campaign name" value={campaignName} onChangeText={setCampaignName} placeholder="Spring booking campaign" /><Field label="Objective" value={campaignObjective} onChangeText={setCampaignObjective} placeholder="Create demand for the selected goal" multiline /><Text style={{ color: brand.text, fontSize: 14, fontWeight: "700" }}>Primary platform</Text><View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>{(["tiktok", "instagram", "facebook", "email", "website"] as const).map((item) => <SecondaryButton key={item} label={item} onPress={() => setCampaignPlatform(item)} disabled={campaignPlatform === item} />)}</View><Text style={{ color: brand.muted, fontSize: 12 }}>Linked goal: {selectedGoal?.title || "None yet"}. Campaigns start as drafts and do not publish automatically.</Text><PrimaryButton label="Save campaign draft" onPress={saveCampaign} loading={createCampaign.isPending} disabled={campaignName.trim().length < 2} icon="check" />{createCampaign.error ? <Notice tone="error">{createCampaign.error.message}</Notice> : null}</View></Surface> : null}
      {campaigns.error ? <Notice tone="error">{campaigns.error.message}</Notice> : null}
      {campaigns.data?.length ? campaigns.data.map((campaign) => <Surface key={campaign.id}><View style={{ flexDirection: "row", justifyContent: "space-between", gap: 12 }}><View style={{ flex: 1, gap: 4 }}><Text style={{ color: brand.text, fontSize: 16, fontWeight: "700" }}>{campaign.name}</Text><Text style={{ color: brand.muted, fontSize: 12 }}>{campaign.objective || "No objective recorded"}</Text></View><StatusPill label={campaign.status} tone="neutral" /></View></Surface>) : <EmptyState title="No campaigns yet" body="Create a draft campaign to connect a goal, content, activity, and Results without inventing performance." />}
      {goals.error ? <Notice tone="error">{goals.error.message}</Notice> : null}
      {selectedGoal ? <><SectionLabel>Current goal</SectionLabel><Surface><View style={{ flexDirection: "row", justifyContent: "space-between", gap: 12 }}><View style={{ flex: 1, gap: 6 }}><Text style={{ color: brand.text, fontSize: 18, fontWeight: "700" }}>{selectedGoal.title}</Text><Text style={{ color: brand.muted, fontSize: 13 }}>{selectedGoal.period} · {selectedGoal.goalType}</Text></View><StatusPill label={selectedGoal.status} tone="success" /></View><View style={{ height: 10, backgroundColor: brand.sand, borderRadius: 999, overflow: "hidden" }}><View style={{ width: `${progress}%`, height: "100%", backgroundColor: brand.rose, borderRadius: 999 }} /></View><View style={{ flexDirection: "row", gap: 12 }}><Metric value={String(selectedGoal.target)} label="Target" /><Metric value={String(selectedGoal.actual)} label="Actual" /><Metric value={`${progress}%`} label="Progress" detail={selectedGoal.actual ? "Recorded" : "No events yet"} /></View></Surface><SectionLabel>Planning lens</SectionLabel><Surface><View style={{ gap: 10 }}><View style={{ flexDirection: "row", justifyContent: "space-between" }}><Text style={{ color: brand.muted, fontSize: 13 }}>Target</Text><Text style={{ color: brand.text, fontSize: 13, fontWeight: "700" }}>{selectedGoal.target}</Text></View><View style={{ flexDirection: "row", justifyContent: "space-between" }}><Text style={{ color: brand.muted, fontSize: 13 }}>Actual</Text><Text style={{ color: brand.text, fontSize: 13, fontWeight: "700" }}>{selectedGoal.actual}</Text></View><View style={{ flexDirection: "row", justifyContent: "space-between" }}><Text style={{ color: brand.muted, fontSize: 13 }}>Remaining if actuals stay current</Text><Text style={{ color: brand.text, fontSize: 13, fontWeight: "700" }}>{planningMath?.remaining}</Text></View><Text style={{ color: brand.muted, fontSize: 12, lineHeight: 18 }}>Forecast: not available until enough performance data exists. Assumptions: the target is user-entered; no conversion rate or revenue guarantee has been inferred.</Text><PrimaryButton label="Create marketing work" onPress={() => router.push("/content" as any)} icon="auto-awesome" /></View></Surface></> : <EmptyState title="No goals yet" body="Add a measurable outcome so marketing work has a direction. Actuals will appear only when real analytics events are recorded." action={<SecondaryButton label="Create your first goal" onPress={() => setShowForm(true)} />} />}
      <SectionLabel>Actual data signal</SectionLabel><Surface><View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}><View style={{ flex: 1, gap: 4 }}><Text style={{ color: brand.text, fontSize: 16, fontWeight: "700" }}>Recorded performance events</Text><Text style={{ color: brand.muted, fontSize: 13, lineHeight: 18 }}>Only connected or manually recorded events appear here.</Text></View><Text style={{ color: brand.text, fontSize: 24, fontWeight: "700" }}>{summary.data?.actualEventCount ?? 0}</Text></View></Surface>
    </View>
  </ScreenShell>;
}
