...
export default function ApprovalsScreen() {
  ...
  return <ScreenShell title="Approval inbox" eyebrow={workspace.name} subtitle="Every sensitive action stays here until a person decides what happens next.">
    <View style={{ gap: 14 }}>
      <Surface tone="ink" style={{ borderColor: brand.border }}><Text style={{ color: brand.text, fontSize: 17, fontWeight: "700" }}>Automation stops at approval.</Text><Text style={{ color: brand.muted, fontSize: 13, lineHeight: 19 }}>Approving a draft changes its workflow state. It does not publish anything unless an authorized connector and schedule are present.</Text></Surface>
      ...
      {approvals.data?.length ? approvals.data.map((approval) => <Surface key={approval.id}><View style={{ flexDirection: "row", justifyContent: "space-between", gap: 12 }}><View style={{ flex: 1, gap: 5 }}><Text style={{ color: brand.text, fontSize: 17, fontWeight: "700" }}>{approval.what}</Text><Text style={{ color: brand.muted, fontSize: 13 }}>{approval.actionType} · {approval.whereTo || "Workspace"}</Text></View><StatusPill label={approval.status} tone={approval.status === "approved" ? "success" : approval.status === "rejected" ? "risk" : "warning"} /></View><Text style={{ color: brand.text, fontSize: 14, lineHeight: 20 }}><Text style={{ fontWeight: "700" }}>Why: </Text>{approval.why || "No reason recorded."}</Text><Text style={{ color: brand.muted, fontSize: 13, lineHeight: 19 }}><Text style={{ fontWeight: "700" }}>Purpose: </Text>{approval.expectedPurpose || "Not provided."}</Text><Text style={{ color: approval.risk === "high" ? brand.ember : brand.muted, fontSize: 12, lineHeight: 18 }}>Risk: {approval.risk} · Approval required: yes</Text>{approval.status === "pending" ? <View style={{ gap: 8, paddingTop: 3 }}><PrimaryButton label="Approve" onPress={() => handle(approval.id, "approved")} loading={decide.isPending} icon="check" /><View style={{ flexDirection: "row", gap: 8 }}><View style={{ flex: 1 }}><SecondaryButton label="Edit" onPress={() => approval.contentItemId ? router.push({ pathname: "/content/detail" as any, params: { id: String(approval.contentItemId) } }) : undefined} icon="edit" /></View><View style={{ flex: 1 }}><SecondaryButton label="Reject" onPress={() => handle(approval.id, "rejected")} icon="close" /></View></View><View style={{ flexDirection: "row", gap: 8 }}><View style={{ flex: 1 }}><SecondaryButton label="Schedule" onPress={() => handle(approval.id, "scheduled")} icon="event" /></View><View style={{ flex: 1 }}><SecondaryButton label="Pause" onPress={() => handle(approval.id, "paused")} icon="pause" /></View></View></View> : null}</Surface>) : <EmptyState title="Inbox is clear" body="No approval requests are waiting in this workspace. New content and sensitive outreach will appear here when they need a human decision." />}
    </View>
  </ScreenShell>;
}
