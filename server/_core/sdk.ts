...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    // Lumière House local email sessions are opaque, database-backed tokens.
    // They are checked before the existing Manus OAuth JWT flow so logout can revoke them.
    if (sessionCookie?.startsWith("lh_")) {
      const localUser = await db.getUserBySessionToken(sessionCookie);
      if (!localUser) throw ForbiddenError("Invalid local session");
