...
export async function apiCall<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  // Prefer the short-lived app session token on every platform. Cookies remain
  // enabled for OAuth and browser sessions, but bearer auth also works when the
  // Expo preview and API are served from different origins.
  const sessionToken = await Auth.getSessionToken();
  console.log("[API] apiCall:", { endpoint, platform: Platform.OS, hasToken: !!sessionToken, method: options.method || "GET" });
  if (sessionToken) {
  ...
  try {
    ...
    console.log("[API] Response received", { status: response.status, requestId: response.headers.get("X-Request-Id") || "missing" });
